...
```{r runcv, message=FALSE, warning=FALSE, echo=FALSE, results='hide'}
...
if(run.cv){
  ...
  for(k in 1:length(model_values)){
    cat("k=", k, "\n")
    err_cv[k,] <- cv.function(
      feat_train, 
      label_train, 
      model_values[k], K #n.trees = 200, bag.fraction = 0.5, inter.depth = 3
      )}
...
set.seed(2018)
path <- "/Users/amon/Documents/GitHub/Fall2018-Proj3-Sec1-grp4"
setwd(path)
```
...
model_values <- c(250,300,350) ### Tuning values
model_labels = paste("GBM with depth =", model_values)
```
...
save(dat_train, file=paste0(
  path,
  "/output/feature_train_xgb.RData"
))
```
...
source(paste0(path, "/lib/train_improved_xgb.R"))
source(paste0(path, "/lib/test_improved_xgb.R"))
```
...
source(paste0(path, "/lib/cross_validation_improved_xgb.R"))
...
if(run.cv){
  err_cv_xgb <- array(dim=c(length(model_values), 2))
  for(k in 1:length(model_values)){
    cat("k=", k, "\n")
    err_cv_xgb[k,] <- cv.function(feat_train, label_train, model_values[k],K)
  }
  save(err_cv_xgb, file=paste0(path, "/output/err_cv_xgb.RData"))
}
```
...
if(run.cv){
  ...
  plot(model_values, err_cv_xgb[,1], xlab="Interaction Depth", ylab="CV Error",
       main="Cross Validation Error", type="n", 
       ylim=c(min(err_cv_xgb[,1])-max(sd(err_cv_xgb[,2]))-0.03, max(err_cv_xgb[,1])+max(sd(err_cv_xgb[,2]))+0.03)
       )
  points(model_values, err_cv_xgb[,1], col="blue", pch=16)
  lines(model_values, err_cv_xgb[,1], col="blue")
  arrows(model_values, err_cv_xgb[,1]-err_cv_xgb[,2], model_values, err_cv_xgb[,1]+err_cv_xgb[,2], 
        length=0.1, angle=90, code=3)
}

# Average cv error and cv SD
apply(err_cv_xgb, 2, mean)
```
...
model_best=model_values[1]
if(run.cv){
  model_best <- model_values[which.min(err_cv_xgb[,1])]
}
