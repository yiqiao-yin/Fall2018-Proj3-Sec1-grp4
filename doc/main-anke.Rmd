...
if(run.cv){
  ...
  plot(model_values, err_cv[,1], xlab="Interaction Depth", ylab="CV Error",
       ...
       ylim=c(min(err_cv[,1])-max(sd(err_cv[,2]))-0.0005, max(err_cv[,1])+max(sd(err_cv[,2]))+0.0005))
...
* Do model selection by choosing among different values of training model parameters, that is, the interaction depth for GBM in this example. 
```{r runcv improved, message=FALSE, warning=FALSE}
...
if(run.cv){
  ...
  for(k in 1:length(model_values)){
    cat("k=", k, "\n")
    err_cv[k,] <- cv.function(feat_train, label_train, K)
  }
